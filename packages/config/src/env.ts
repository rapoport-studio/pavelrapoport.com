...
const envSchema = z
  .object({
    ...
    // to access studio.pavelrapoport.com (required in production)
    STUDIO_ALLOWED_EMAILS: z.string().optional(),
  })
  .refine(
    (data) => {
      if (data.NODE_ENV === "production") {
        return (
          typeof data.STUDIO_ALLOWED_EMAILS === "string" &&
          data.STUDIO_ALLOWED_EMAILS.trim().length > 0
        );
      }
      return true;
    },
    {
      message: "STUDIO_ALLOWED_EMAILS must be set in production",
      path: ["STUDIO_ALLOWED_EMAILS"],
    }
  );
